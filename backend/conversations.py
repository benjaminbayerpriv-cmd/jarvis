...
def append_turn(conv_id: str, user_text: str, assistant_text: str) -> dict:
    ...
    with _lock:
        ...
        if conv is None:
            conv = {"id": conv_id, "title": None, "created_at": now, "turns": []}
...
def list_conversations() -> list[dict]:
    ...
    never needs the full turn list just to render its entries."""
    if not DIR.exists():
        return []
    out = []
